...
class IncidentSeverity(str, enum.Enum): INFO="INFO"; LOW="LOW"; MEDIUM="MEDIUM"; HIGH="HIGH"; CRITICAL="CRITICAL"
class IncidentStatus(str, enum.Enum): OPEN="OPEN"; RESOLVED="RESOLVED"
class User(Base):
...
class Incident(Base):
    ...
    id:Mapped[int]=mapped_column(primary_key=True); public_id:Mapped[str|None]=mapped_column(String(30),unique=True,index=True)
    bot_id:Mapped[str|None]=mapped_column(ForeignKey("bots.id"),index=True); scope:Mapped[str]=mapped_column(String(10),default="BOT",index=True)
    incident_type:Mapped[str]=mapped_column(String(40),index=True); severity:Mapped[IncidentSeverity]=mapped_column(Enum(IncidentSeverity),index=True); status:Mapped[IncidentStatus]=mapped_column(Enum(IncidentStatus),default=IncidentStatus.OPEN,index=True)
    title:Mapped[str]=mapped_column(String(200)); summary:Mapped[str]=mapped_column(String(1000)); source:Mapped[str]=mapped_column(String(40)); fingerprint:Mapped[str]=mapped_column(String(64),index=True)
    started_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=utcnow,index=True); last_updated_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=utcnow); resolved_at:Mapped[datetime|None]=mapped_column(DateTime(timezone=True)); resolution:Mapped[str|None]=mapped_column(String(30)); current_instance_id:Mapped[str|None]=mapped_column(String(41)); context:Mapped[dict]=mapped_column(JSON,default=dict); error_group_id:Mapped[int|None]=mapped_column(ForeignKey("error_groups.id"))
class IncidentEvent(Base):
    __tablename__="incident_events"; __table_args__=(UniqueConstraint("source_key","event_code",name="uq_incident_source_key"),)
    id:Mapped[int]=mapped_column(primary_key=True); incident_id:Mapped[int]=mapped_column(ForeignKey("incidents.id"),index=True); source_event_id:Mapped[int|None]=mapped_column(ForeignKey("activity_events.id")); source_key:Mapped[str]=mapped_column(String(36)); timestamp:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=utcnow,index=True); event_code:Mapped[str]=mapped_column(String(100)); label:Mapped[str]=mapped_column(String(200)); detail:Mapped[str|None]=mapped_column(String(1000)); metadata_snapshot:Mapped[dict]=mapped_column(JSON,default=dict)
class ErrorGroup(Base):
    __tablename__="error_groups"; __table_args__=(UniqueConstraint("bot_id","fingerprint",name="uq_error_group_bot_fingerprint"),)
    id:Mapped[int]=mapped_column(primary_key=True); public_id:Mapped[str|None]=mapped_column(String(30),unique=True,index=True); bot_id:Mapped[str]=mapped_column(ForeignKey("bots.id"),index=True); fingerprint:Mapped[str]=mapped_column(String(64),index=True); exception_type:Mapped[str|None]=mapped_column(String(100)); safe_signature:Mapped[str]=mapped_column(String(500)); source:Mapped[str]=mapped_column(String(80)); first_seen:Mapped[datetime]=mapped_column(DateTime(timezone=True)); last_seen:Mapped[datetime]=mapped_column(DateTime(timezone=True)); occurrence_count:Mapped[int]=mapped_column(Integer,default=1); latest_incident_id:Mapped[int|None]=mapped_column(ForeignKey("incidents.id"))
