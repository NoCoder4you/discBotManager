...
async def bot_status(bot:Bot=Depends(requires_bot_permission("bot.view"))):
    health=await process_manager.get_status(bot.id,bot.enabled); return {"state":health.state.value,"process_running":health.process_running,"discord_connected":health.discord_connected,"discord_ready":health.discord_ready,"detail":health.detail}
